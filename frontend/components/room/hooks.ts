...
export const useRoom = (roomId: number): [Room, boolean, boolean] => {
  ...
  useEffect(() => {
    if (!roomId) return;
    const subscriptionRoom = roomService.room.subscribe((room) =>
      setRoom(room)
    );
    const subscriptionLoading = roomService.isLoading.subscribe((isLoading) =>
      setIsLoading(isLoading)
    );
    const subscriptionFailed = roomService.failed.subscribe((failed) =>
      setFailed(failed)
    );
...
export const useRooms = (): [Room[], boolean, boolean] => {
  ...
  useEffect(() => {
    const subscriptionRoom = roomsService.rooms.subscribe((room) =>
      setRoom(room)
    );
    const subscriptionLoading = roomsService.isLoading.subscribe((isLoading) =>
      setIsLoading(isLoading)
    );
    const subscriptionFailed = roomsService.failed.subscribe((failed) =>
      setFailed(failed)
    );
